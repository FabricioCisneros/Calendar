    ...
    <main class="flex-1 relative overflow-y-auto py-6 focus:outline-none" tabindex="0">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-5">
            <div class="md:flex md:items-center md:justify-between">
                <div class="flex-1 min-w-0">
                    <h1 class="py-0.5 text-2xl font-semibold text-gray-900">Oficinas</h1>
                </div>
            </div>
        </div>
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="my-6 bg-white shadow overflow-hidden sm:rounded-md">
                ...
                <template v-if="oficinalist.length > 0">
                    <ul>
                        <template v-for="(oficina, index) in oficinalist">
                            <li :class="{'border-t border-gray-200': index !== 0}">
                                    <div class="flex items-center px-4 py-4 sm:px-6">
                                        <div class="min-w-0 flex-1 flex items-center">
                                            <div class="min-w-0 flex-1 px-4 lg:grid lg:grid-cols-2 lg:gap-4">
                                                <div class="flex">
                                                    <div class="inline-block align-middle text-sm font-medium leading-5 text-gray-900">{{ oficina.descripcion }}</div>
                                                </div>
                                            </div>
                                        </div>
                                        <div>
...
export default {
    ...
    methods: {
        getOffices() {
            const self = this;
            self.loading = true;
            axios.get('api/dashboard/admin/oficinas').then(function (response) {
                self.oficinalist = response.data;
                self.loading = false;
            }).catch(function () {
                self.loading = false;
            });
