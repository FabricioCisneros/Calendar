    ...
    <main class="flex-1 relative overflow-y-auto py-6 focus:outline-none" tabindex="0">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-5">
            <div class="md:flex md:items-center md:justify-between">
                <div class="flex-1 min-w-0">
                    <h1 class="py-0.5 text-2xl font-semibold text-gray-900">{{ $t('Categoria') }}</h1>
                </div>
                <div class="mt-4 flex md:mt-0 md:ml-4">
                    <router-link
                        class="btn btn-blue shadow-sm rounded-md"
                        to="/dashboard/admin/categorias/new"
                    >
                        {{ $t('Agregar nueva Categoria') }}
                    </router-link>
                </div>
            </div>
        </div>
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="my-6 bg-white shadow overflow-hidden sm:rounded-md">
                ...
                <template v-if="categoriaList.length > 0">
                    <ul>
                        <template v-for="(category, index) in categoriaList">
                            <li :class="{'border-t border-gray-200': index !== 0}">
                                <router-link
                                    :to="'/dashboard/admin/categorias/' + category.id + '/edit'"
                                    ...
                                    <div class="flex items-center px-4 py-4 sm:px-6">
                                        <div class="min-w-0 flex-1 flex items-center">
                                            <div class="flex-shrink-0">
                                                <svg-vue :style="{color: category.color}" class="h-5 w-5" icon="font-awesome.circle-solid"></svg-vue>
                                            </div>
    ...
    methods: {
        getCategories() {
            ...
            axios.get('api/dashboard/admin/categorias').then(function (response) {
                ...
                self.categoriaList = response.data;
                self.loading = false;
            }).catch(function () {
                self.loading = false;
            });
