<template>
    <main class="flex-1 relative overflow-y-auto py-6 focus:outline-none" tabindex="0">
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 px-5">
                <div class="md:flex md:items-center md:justify-between">
                    <div class="flex-1 min-w-0">
                        <h1 class="py-0.5 text-2xl font-semibold text-gray-900">{{ $t('Editar turno') }}</h1>
                    </div>
                    <div class="mt-4 flex md:mt-0 md:ml-4">
                        <button
                            class="btn btn-red shadow-sm rounded-md"
                            type="button"
                            @click="deleteTurnoModal = true"
                        >
                            {{ $t('Borrar turno') }}
                        </button>
                    </div>
                ...
                <div class="mt-6 shadow sm:rounded-lg">
                    ...
                    <div class="bg-white md:grid md:grid-cols-3 md:gap-6 px-4 py-5 sm:p-6">
                        <div class="md:col-span-1">
                            <h3 class="text-lg font-medium leading-6 text-gray-900">{{ $t('Informacion del turno') }}</h3>
                        </div>
        ...
        <div v-show="deleteTurnoModal" class="fixed z-20 inset-0 overflow-y-auto">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                >
                    <div v-show="deleteTurnoModal" class="fixed inset-0 transition-opacity">
                                ...
                                <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                    ...
                                    <div class="mt-2">
                                        <p class="text-sm leading-5 text-gray-500">
                                            {{ $t('Estas seguro de eliminar este turno?') }}
...
export default {
    ...
    metaInfo() {
        return {
            title: this.$i18n.t('Edicion de turnos')
        }
    },
    data() {
        return {
            loading: true,
            deleteTurnoModal: false,
            selectedState: '',
            turnos:{
                name: null,
            },
    ...
    methods: {
        saveTurno() {
            const self = this;
            self.loading = true;
            axios.put('api/dashboard/admin/turnos/' + self.$route.params.id, self.turnos).then(function () {
                self.loading = false;
                self.$notify({
        ...
        getTurno() {
            const self = this;
            self.loading = true;
            axios.get('api/dashboard/admin/turnos/' + self.$route.params.id).then(function (response) {
        ...
        deleteTurno() {
            const self = this;
            axios.delete('api/dashboard/admin/turnos/' + self.$route.params.id).then(function () {
                self.$notify({
                    title: self.$i18n.t('completado').toString(),
